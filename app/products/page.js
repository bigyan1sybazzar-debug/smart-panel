    ...
    <div>
      ...
      <div className="container-page py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {products.map((p) => (
            <div key={p.id} className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl overflow-hidden hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between">
              <div>
                <div className="h-56 overflow-hidden relative bg-gray-50">
                  ...
                  />
                  <span className="absolute top-3 left-3 bg-brand-blue text-white text-[11px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-md shadow-sm">
                    {p.category}
                  </span>
                </div>
                <div className="p-6">
                  <span className="text-xs font-bold text-brand-orange uppercase tracking-wider">{p.category}</span>
                  <h2 className="font-display font-bold text-lg text-brand-blue-dark mt-1 group-hover:text-brand-blue transition-colors">
                    {p.name}
                  </h2>
              ...
              <div className="p-6 pt-0">
                <Link
                  href={`/products/${p.id}`}
                  className="inline-flex items-center justify-center w-full bg-brand-cream hover:bg-brand-blue hover:text-white text-brand-blue-dark font-bold text-xs py-2.5 rounded-lg border border-blue-900/10 transition-colors"
                >
